...
export class MissionRequest implements OnInit {
  ...
  getProgressWidth(): number {
    if (this.missionForm.valid) return 100;
    if (this.selectedWorkers.length > 0 && this.missionForm.get('destination')?.value) return 66;
    if (this.selectedWorkers.length > 0) return 33;
    return 0;
  }

  // Calculate duration between dates
  calculateDuration(): number {
    const start = this.missionForm.get('missionDate')?.value;
    const end = this.missionForm.get('endDate')?.value;
  ...
  // Get current step for mobile view
  getCurrentStep(): number {
    if (!this.selectedWorkers || this.selectedWorkers.length === 0) return 1;
    if (!this.missionForm.get('destination')?.value) return 2;
    return 3;
  }
