...
export class AddWorkerComponent implements OnInit {
  ...
  readonly MAX_FILE_SIZE = 10 * 1024 * 1024; // 5MB
  ...
  readonly ALLOWED_IMAGE_TYPES = ['image/jpeg', 'image/png'];
  ...
  onFileSelected(event: Event): void {
    ...
    if (input.files && input.files[0]) {
      ...
      if (file.size > MAX_FILE_SIZE) {
        this.notificationService.showError('La photo de profil ne doit pas dépasser 10 Mo');
        return;
      }
  ...
  private initializeFormWithWorkerData(worker: Worker): void {
    ...
    this.workerForm.patchValue({
      ...
      startDate: worker.creationDate ? new Date(worker.creationDate).toISOString().split('T')[0] : '',
      totalCongeDays: worker.totalCongeDays
    });

    // Load profile photo if exists
    if (worker.profileImagePath) {
      this.imagePreview = worker.profileImagePath;
    }
