...
export class DemandeAvanceComponent {
  ...
  calculatePercentage(): number {
    if (!this.demande.requestedAmount || !this.workerSalary || this.workerSalary === 0) {
      return 0;
    }
    return (this.demande.requestedAmount / this.workerSalary) * 100;
  }

  onAmountSliderChange(event: Event): void {
    const value = (event.target as HTMLInputElement).value;
    this.demande.requestedAmount = parseInt(value, 10);
  }

  setAmountByPercentage(percentage: number): void {
    if (this.workerSalary) {
      this.demande.requestedAmount = Math.round((this.workerSalary * percentage) / 100);
    }
  }

  // Update the onCinChange method to reset amount when CIN changes
  onCinChange(): void {
    this.workerName = null;
    this.cinError = null;
    this.workerId = null;
    this.workerSalary = null;
    this.demande.requestedAmount = null;
